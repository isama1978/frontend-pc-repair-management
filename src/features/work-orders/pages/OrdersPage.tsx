...
> = {
  ...
  PRESUPUESTADO: "budgeting",
  ESPERANDO_REPUESTO: "waitingForPart",
  REPARANDO: "repair",
  TESTEANDO: "test",
  LISTO: "ready",
  ENTREGADO: "delivered",
...
export const OrdersPage: React.FC = () => {
  ...
  const lang = useSelector(
    (state: RootState) => state.ui.language as "es" | "en" | "pt",
  );
  const t = uiDictionaries[lang].orders || uiDictionaries["es"].orders;

  const { orders, loading, error } = useSelector(
    (state: RootState) => state.workOrders,
  );
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order: WorkOrder) => {
      ...
      const matchDni =
        searchDni === "" ||
        order.clientId.toLowerCase().includes(searchDni.toLowerCase());
      const matchSerial =
        searchSerial === "" ||
        (order.serialNumber &&
          order.serialNumber
      ...
      <Box sx={{ my: 4 }}>
        ...
        >
          <Typography sx={{ variant: "h4", fontWeight: "bold" }} component="h1">
            {viewMode === "all"
              ? t.allOrdersTitle || "Órdenes de Trabajo"
              : t.myOrdersTitle || "Mis Órdenes Asignadas"}
          </Typography>
        ...
        <Paper elevation={2} sx={{ p: 3, mb: 4 }}>
          <Grid container spacing={2} sx={{ alignItems: "center" }}>
            <Grid size={{ xs: 12, sm: 4 }}>
              ...
              >
                <MenuItem value="ALL">
                  {t.statusAll || "Todos los estados"}
                </MenuItem>
                <MenuItem value="RECIBIDO">{t.received || "Recibido"}</MenuItem>
                <MenuItem value="DIAGNOSTICANDO">
                  {t.diagnostic || "Diagnosticando"}
                </MenuItem>
                <MenuItem value="PRESUPUESTADO">
                  {t.budgeting || "Presupuestado"}
                </MenuItem>
                <MenuItem value="ESPERANDO_REPUESTO">
                  {t.waitingForPart || "Esperando Repuesto"}
                </MenuItem>
            ...
            <Table sx={{ minWidth: 650 }} aria-label="work orders table">
              <TableHead sx={{ backgroundColor: "background.default" }}>
                <TableRow>
                  ...
                  <TableCell sx={{ fontWeight: "bold" }}>
                    {t.thEquipment || "Equipo"}
                  </TableCell>
                  <TableCell sx={{ fontWeight: "bold" }}>
                    {t.thFailure || "Falla Reportada"}
                  </TableCell>
                  <TableCell sx={{ fontWeight: "bold" }}>
                    {t.thStatus || "Estado"}
                  </TableCell>
                  <TableCell sx={{ fontWeight: "bold" }} align="right">
                    {t.thTotal || "Monto Total"}
                  </TableCell>
                  <TableCell sx={{ fontWeight: "bold" }} align="center">
                    {t.thActions || "Acciones"}
                  </TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {filteredOrders.length === 0 ? (
                  ...
                  filteredOrders.map((order) => {
                    ...
                    const translationKey = statusTranslationKeys[order.status];
                    const translatedStatusLabel = translationKey
                      ? t[translationKey]
                      : order.status;
                    return (
                      <TableRow
